...
public class BinTree<K, V> {
	...
	private void printReverseSubTree(Node<K, V> node) {
		if (node != null) {
			printReverseSubTree(node.right); // 오른쪽 서브 트리를 키 값의 내림차순으로 출력
			System.out.println(node.key + " " + node.data); // node를 출력
			printReverseSubTree(node.left); // 왼쪽 서브 트리를 키 값의 내림차순으로 출력
		}
	}
