...
public class G1_13459_구슬탈출 {
	...
	// 빨간 구슬, 파란 구슬의 좌표와 이동 횟수를 나타내는 클래스
	static class Marble {
		int rx, ry, bx, by, cnt;
	...
	static boolean bfs() {
		...
		while (!queue.isEmpty()) {
			...
			int ncnt = cur.cnt + 1; // 카운트 증가
			
			if (ncnt > 10)	// 카운트가 10을 초과하면 구슬을 움직이지 않고 넘어감
				continue;
			...
			for (int d = 0; d < 4; d++) { // 사방으로 기울임
				...
				if (!visit[nrx][nry][nbx][nby]) { // 해당 좌표에 방문하지 않았다면
					...
					queue.add(new Marble(nrx, nry, nbx, nby, ncnt));
				}
			}
			
		}
		
		return false; // 10회 이내 빨간 구슬이 구멍에 들어가지 못했으므로 false 리턴
	}
}
