package com.jm0nn.baekjoon.platinum.p2;
...
// RPG 게임
public class P2_17081_RPGExtreme {
	...
	static class Player {
		...
		Player(int r, int c) {
			...
			this.lv = 1; // 레벨 1
			this.hpMax = 20; // 최대 체력 20
			this.hp = this.hpMax; // 체력 20
			this.att = 2; // 공격력 2
			this.def = 2; // 방어력 2
			this.expReq = 5 * this.lv; // 요구 경험치 5
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < command.length; i++, T++) {
			...
			// 가시를 밟았을 때
			if (pos == '^') {
				// Dexterity 장신구를 착용했을 경우 가시 데미지 1
	...
	static void fight(int r, int c) {
		...
		while (true) {
			if (firstAtt) { // 첫 번째 공격일 경우
				...
				mHp -= Math.max(1, fAtt - mDef); // 플레이어의 공격

				firstAtt = false;
			} else {
				mHp -= Math.max(1, pAtt - mDef); // 플레이어의 공격
			}
			...
			player.hp -= Math.max(1, mAtt - pDef); // 몬스터의 공격
	...
	static void fightBoss(int r, int c) {
		...
		// Hunter 장신구를 착용했을 경우 체력을 최대치까지 회복, 보스의 첫 번째 공격 데미지 0
