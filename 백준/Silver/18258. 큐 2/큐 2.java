...
public class Main {
	...
	public static void main(String[] args) {
		...
		while (n-- > 0) {
			switch (fio.next()) {
			case "push":
				queue[tail++] = fio.nextInt();
				break;

			case "pop":
				fio.write(head != tail ? queue[head++] : -1).newLine();
				break;

			case "size":
				fio.write(tail - head).newLine();
				break;

			case "empty":
				fio.write(head == tail ? 1 : 0).newLine();
				break;
			...
			case "back":
				fio.write(head != tail ? queue[tail - 1] : -1).newLine();
				break;
			}
		}
